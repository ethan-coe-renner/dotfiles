...
(use-package emacs
  ...
  ;; Accept larger responses from subprocesses so we don't need to make as many read calls
...
Use cape for completion at point
#+begin_src emacs-lisp
(use-package cape
  :straight t
  :general
...
#+begin_src emacs-lisp
(setq machine-specific-configuration-directory (expand-file-name "~/git/config/elisp"))
